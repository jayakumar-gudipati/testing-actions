...
previous_days_successful = spark.read.schema(previous_days_successful_schema).parquet(f"s3://snowbergsandbox/zeus_disb_format/successful_list_new/", InferSchema=True).alias("pds")

previous_day_unsuccessful = spark.read.parquet("s3://snowbergsandbox/zeus_disb_format/unsuccessful_list/")
...
fl = spark.read.parquet("s3://snowbergdatalanding/stg_vcl_colending_yuserve/fact_loans", InferSchema = True).filter(
    (f.to_date(f.col('vcl_updated_at'), 'yyyy-MM-dd')==date_filter) | (f.col('id').isin([row["loan_id"] for row in previous_day_unsuccessful.select("loan_id").collect()]))).select("*", 
                                                                                                                    f.to_date(f.col("client_disbursed_date"),'yyyy-MM-dd').alias("cdd_new"),
                                                                                                                    f.to_date(f.col("first_repayment_date"),'yyyy-MM-dd').alias("frd_new"),
                                                                                                                    f.to_date(f.col("agreement_signed_date"),'yyyy-MM-dd').alias("asd_new"),
                                                                                                                    f.to_date(f.col("disbursed_date"),'yyyy-MM-dd').alias("dd_new"),
                                                                                                                   ).alias("fl")

today_updated_loans_df = fl.join(previous_days_successful, previous_days_successful.loan_id==fl.id, "left_anti").select(f.col("fl.*")).alias("fl")
...
fpro = spark.read.parquet("s3://snowbergdatalanding/stg_vcl_colending_yuserve/fact_products/", InferSchema=True, pathGlobfilter="*.parquet").filter((f.col("is_deleted")==False) & (f.col("client_namespace_internal").isin(['true', 'whizdm', 'smartcoin', 'moneyboxx', 'axio', 'flexiloan']))).alias("fpro")

fda = spark.read.parquet("s3://snowbergdatalanding/stg_vcl_colending_yuserve/fact_disbursement_accounts/", InferSchema = True).filter((f.col("is_deleted")==False) & (f.col("accountable_type")=='VivColendingApi::Loan') & (f.col("account_type")==0)).alias("fda")

frs = spark.read.parquet("s3://snowbergdatalanding/stg_vcl_colending_yuserve/fact_repayment_schedules/", InferSchema=True, pathGlobfilter="*.parquet").filter(f.col("is_deleted")==False).alias("frs")
...
today_updated_loans_df = today_updated_loans_df.filter(fl.dd_new >= cutoff_date)
...
state_name_mapping_df = spark.read.option("InferSchema", True).option("header", True).csv("s3://snowbergdatalanding/state_name_mapping/mapping_state_names.csv").alias("snm")

# taking distinct rows
snm = state_name_mapping_df.select(f.col("State_Derv"), f.col("State"), f.col("Zone")).distinct().alias("snm")
...
joins = today_updated_loans_df.alias("fl").join(fpro, today_updated_loans_df.product_id == fpro.id, "inner")\
    ...
    f.regexp_replace(fb.middle_name, '[^a-zA-Z0-9]\s', '').alias("middle_name"),
    f.when(last_name_expr.isNotNull(), f.regexp_replace(last_name_expr, "[^a-zA-Z0-9]\s", ""))
    .otherwise(None).alias("last_name"),
    f.round(f.coalesce(f.col("fb.age"),
        f.floor(f.datediff(f.col("fl.cdd_new"), f.col("fb.dob_new")) / 365)
    ),0).alias("age"),
    f.when(f.lower(fb.gender).isin(['male', 'm', 'mele', 'male&nbsp;']), 'Male')
    ...
    f.regexp_replace(fb.passport_number, '[^a-zA-Z0-9]', '').alias("Passport Number"),
    f.regexp_replace(fb.driving_license_number, '[^a-zA-Z0-9]', '').alias("Driving License"),
    f.concat(f.lit("'"), f.coalesce(
        f.when((f.length(f.trim(fb.preferred_address)) > 1) & ~(fb.preferred_address.startswith("[B%")), fb.preferred_address)
    ...
    f.coalesce(fb.permanent_pincode, fb.current_pincode).cast("string").alias("Pin Code"),
    f.regexp_replace(fb.mobile_number, '[^0-9]', '').cast("string").alias("Mobile Number"),
    f.when(f.length(f.trim(fb.email)) > 4, fb.email).cast("string").alias("Email Address"),
    f.regexp_replace(
        f.coalesce(
    ...
    f.when(~snm.State_Derv.isin(['zBlank', 'others']), 
           ...
           .otherwise(snm.State))
    .alias("State"),
    f.date_format(today_updated_loans_df.cdd_new, 'dd-MM-yyyy').alias("Submitted On"),
    f.date_format(today_updated_loans_df.cdd_new, 'dd-MM-yyyy').alias("Activation Date"),
    f.regexp_replace(f.coalesce(fda.account_name, f.concat(f.trim(fb.first_name), f.lit(" "), f.trim(fb.middle_name), f.lit(" "), f.trim(fb.last_name))), '[^a-zA-Z0-9]\s', '').substr(1,50).alias("Beneficiary Name"),
    f.regexp_replace(fda.account_no,'[^a-zA-Z0-9]','').cast("string").alias("Beneficiary Account No"),
    ...
    f.date_format(today_updated_loans_df.cdd_new, 'dd-MM-yyyy').alias("Loan Submitted On"),
    f.date_format(today_updated_loans_df.asd_new, 'dd-MM-yyyy').alias("agreement signed date"),
    f.date_format(today_updated_loans_df.cdd_new, 'dd-MM-yyyy').alias("Disbursed Date"),
    today_updated_loans_df.principal_amount.alias("Principal"),
    today_updated_loans_df.tenure.alias("Loan Term"),
    f.date_format(today_updated_loans_df.frd_new,'dd-MM-yyyy').alias("First Repayment On"),
#     f.day(fl.first_repayment_date).alias("Due Day"),
    ...
    f.coalesce(today_updated_loans_df.processing_fee, f.lit(0)).alias("Processing Fee"),
    f.coalesce(today_updated_loans_df.nach_charges, f.lit(0)).alias("Nach Charges"),
    f.coalesce(today_updated_loans_df.dcc_charges, f.lit(0)).alias("DCC Charges"),
    f.coalesce(today_updated_loans_df.stamp_duty, f.lit(0)).alias("Stamp Duty"),
    f.coalesce(today_updated_loans_df.documentation_charges, f.lit(0)).alias("Documentation Charges"),
    f.coalesce(today_updated_loans_df.other_charges, f.lit(0)).alias("Other Charges"),
    f.coalesce(today_updated_loans_df.insurance_charges, f.lit(0)).alias("Insurance Charges"),
    ...
    f.greatest(today_updated_loans_df.etl_updated_at, fpro.etl_updated_at, fb.etl_updated_at, fda.etl_updated_at).alias("max_etl_updated_at_from_tbls"),
...
unsuccessful_df.coalesce(1).write.mode("overwrite").parquet("s3://snowbergsandbox/zeus_disb_format/unsuccessful_list/")

successful_df = joins.join(unsuccessful_df, joins.loan_id == unsuccessful_df.loan_id, "left_anti").select("joins.*")


successful_df.coalesce(1).write.mode("overwrite").parquet(f"s3://snowbergsandbox/zeus_disb_format/successful_list_new/{current_date}/")